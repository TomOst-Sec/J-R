"""Pinterest platform module — visual discovery and bookmarking."""
...
class PinterestPlatform(BasePlatform):
    """Pinterest platform for visual discovery profiles."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Pinterest username exists."""
        url = f"https://www.pinterest.com/{quote(username)}/"
        try:
            async with self.session.get(url, allow_redirects=True) as resp:
                ...
                if resp.status == 200:
                    ...
                    if username.lower() in text.lower():
                        return True
                    return None
                # May be blocked — try browser fallback
        except Exception:
            pass
    ...
    async def search_name(
        ...
        """Search for Pinterest users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Pinterest profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://www.pinterest.com/{quote(username)}/"
        html = None
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Pinterest profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Pinterest username from a URL."""
