"""Spotify platform module — music profile discovery."""
...
class SpotifyPlatform(BasePlatform):
    """Spotify platform for music profile discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Spotify username exists."""
    ...
    async def search_name(
        ...
        """Search for Spotify users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Spotify profile page."""
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Spotify profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Spotify username from a URL."""
        match = re.search(r"open\.spotify\.com/user/([^/?]+)", url)
        return match.group(1) if match else None
