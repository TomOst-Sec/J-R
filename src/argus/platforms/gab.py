"""Gab platform module — alternative social network discovery."""
...
class GabPlatform(BasePlatform):
    """Gab platform for alternative social network profile discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Gab username exists."""
        url = f"https://gab.com/{quote(username)}"
        try:
    ...
    async def search_name(
        ...
        """Search for Gab users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Gab profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://gab.com/{quote(username)}"
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Gab profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Gab username from a URL."""
        match = re.search(r"gab\.com/([^/?]+)", url)
        return match.group(1) if match else None
