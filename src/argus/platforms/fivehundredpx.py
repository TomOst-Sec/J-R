"""500px platform module — photography portfolio discovery."""
...
class FiveHundredPxPlatform(BasePlatform):
    """500px platform for photography portfolio discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a 500px username exists."""
    ...
    async def search_name(
        ...
        """Search for 500px users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a 500px profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://500px.com/p/{username}"
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse 500px profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract 500px username from a URL."""
        match = re.search(r"500px\.com/p/([^/?]+)", url)
        return match.group(1) if match else None
