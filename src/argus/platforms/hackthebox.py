"""HackTheBox platform module — cybersecurity profile discovery."""
...
class HackTheBoxPlatform(BasePlatform):
    """HackTheBox platform for cybersecurity profile discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a HackTheBox user exists."""
        url = f"https://app.hackthebox.com/users/{quote(username)}"
        try:
    ...
    async def search_name(
        ...
        """Search for HackTheBox users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a HackTheBox profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://app.hackthebox.com/users/{quote(username)}"
        html = None
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse HackTheBox profile from page content."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract HackTheBox username from a URL."""
        match = re.search(r"hackthebox\.com/users/([^/?]+)", url)
        return match.group(1) if match else None
