"""Strava platform module — fitness/athletics profile discovery."""
...
class StravaPlatform(BasePlatform):
    """Strava platform for athlete profile discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Strava athlete exists."""
        url = f"https://www.strava.com/athletes/{quote(username)}"
        try:
    ...
    async def search_name(
        ...
        """Search for Strava athletes — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Strava athlete profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://www.strava.com/athletes/{quote(username)}"
        html = None
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Strava profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Strava username from a URL."""
        match = re.search(r"strava\.com/athletes/([^/?]+)", url)
        return match.group(1) if match else None
