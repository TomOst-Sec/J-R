"""Dribbble platform module — design portfolio discovery."""
...
class DribbblePlatform(BasePlatform):
    """Dribbble platform for design portfolio discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Dribbble username exists."""
    ...
    async def search_name(
        ...
        """Search for Dribbble users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Dribbble profile page."""
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Dribbble profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Dribbble username from a URL."""
        match = re.search(r"dribbble\.com/([^/?]+)", url)
        return match.group(1) if match else None
