"""DeviantArt platform module — art portfolio discovery."""
...
class DeviantArtPlatform(BasePlatform):
    """DeviantArt platform for art portfolio discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a DeviantArt username exists."""
    ...
    async def search_name(
        ...
        """Search for DeviantArt users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a DeviantArt profile page."""
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse DeviantArt profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract DeviantArt username from a URL."""
        match = re.search(r"deviantart\.com/([^/?]+)", url)
        return match.group(1) if match else None
