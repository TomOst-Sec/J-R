"""Substack platform module — newsletter/blog discovery."""
...
class SubstackPlatform(BasePlatform):
    """Substack platform for newsletter/blog discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Substack username exists."""
    ...
    async def search_name(
        ...
        """Search for Substack users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Substack profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://{username}.substack.com"
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Substack profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Substack username from a URL."""
        # Handle subdomain pattern: {username}.substack.com
        match = re.search(r"([^/.]+)\.substack\.com", url)
        if match:
            return match.group(1)
