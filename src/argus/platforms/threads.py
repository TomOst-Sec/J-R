"""Threads platform module — Meta text-based social discovery."""
...
class ThreadsPlatform(BasePlatform):
    """Threads (Meta) platform for social profile discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Threads username exists."""
        url = f"https://www.threads.net/@{quote(username)}"
        try:
    ...
    async def search_name(
        ...
        """Search for Threads users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Threads profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://www.threads.net/@{quote(username)}"
        html = None
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Threads profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Threads username from a URL."""
