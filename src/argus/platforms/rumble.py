"""Rumble platform module — alternative video platform discovery."""
...
class RumblePlatform(BasePlatform):
    """Rumble platform for alternative video profile discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Rumble username exists."""
        url = f"https://rumble.com/user/{quote(username)}"
        try:
    ...
    async def search_name(
        ...
        """Search for Rumble users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Rumble profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://rumble.com/user/{quote(username)}"
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Rumble profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Rumble username from a URL."""
        match = re.search(r"rumble\.com/user/([^/?]+)", url)
        return match.group(1) if match else None
