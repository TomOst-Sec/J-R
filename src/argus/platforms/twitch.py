"""Twitch platform module — live streaming profile discovery."""
...
class TwitchPlatform(BasePlatform):
    """Twitch platform for streaming profile discovery."""
        ...
        try:
            async with self.session.get(url, allow_redirects=True) as resp:
                ...
                if resp.status == 200:
                    ...
                    # Twitch shows this message for nonexistent users
                    if "Sorry. Unless you've got a time machine" in text:
                        return False
    ...
    async def search_name(
        ...
        """Search for Twitch users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Twitch profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://www.twitch.tv/{quote(username)}"
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Twitch profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Twitch username from a URL."""
