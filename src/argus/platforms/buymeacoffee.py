"""Buy Me a Coffee platform module — creator support page discovery."""
...
class BuyMeACoffeePlatform(BasePlatform):
    """Buy Me a Coffee platform for creator support page discovery."""
    ...
    async def check_username(self, username: str) -> bool | None:
        """Check if a Buy Me a Coffee username exists."""
    ...
    async def search_name(
        ...
        """Search for BMC users — returns empty (no public search API)."""
        return []

    async def scrape_profile(self, url: str) -> ProfileData | None:
        """Scrape a Buy Me a Coffee profile page."""
        username = self._extract_username(url)
        if not username:
            return None

        profile_url = f"https://www.buymeacoffee.com/{username}"
        try:
    ...
    def _parse_profile(self, html_text: str, username: str) -> ProfileData | None:
        """Parse Buy Me a Coffee profile from HTML/meta tags."""
    ...
    def _extract_username(url: str) -> str | None:
        """Extract Buy Me a Coffee username from a URL."""
        match = re.search(r"buymeacoffee\.com/([^/?]+)", url)
        return match.group(1) if match else None
